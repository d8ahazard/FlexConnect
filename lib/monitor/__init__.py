...
from OSHelper import get_os_name
...
class Monitor(object):
    ...
    def get_memory(cls):
        system_name = get_os_name()
        if system_name == "Windows":
            mem_total = float(run_command("wmic ComputerSystem get totalPhysicalMemory")[1])
            mem_free = float(run_command("wmic OS get freePhysicalMemory")[1]) * 1024
            mem_used = float(mem_total - mem_free) + .0
    ...
    def get_disk(cls):
        ...
        os_name = get_os_name()
        if os_name == "Windows":
            disk_data = run_command('powershell -Command " & {get-psdrive -psprovider filesystem | select Name, {$_.Used /1GB}, {$_.Free /1GB}, Root}"')
        else:
        ...
        for line in disk_data:
            ...
            if get_os_name() == "Windows":
                ...
                log.debug("Used %s total %s free %s" % (used_size, total_size, free_size))
                percent = 0
                if (used_size > 0.0) & (total_size > 0.0):
